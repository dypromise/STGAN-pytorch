...
class STGANAgent(object):
    def __init__(self, config):
        ...
        self.D = Discriminator(self.config.image_size,
                               len(self.config.attrs),
                               self.config.d_conv_dim,
                               self.config.d_fc_dim,
    ...
    def train(self):
        ...
        for i in range(self.current_iteration, self.config.max_iters):
            ...
            d_loss = d_loss_gan + self.config.lambda_gp * d_loss_gp + \
                self.config.lambda1 * d_loss_gan
            ...
            if self.current_iteration % self.config.summary_step == 0:
                ...
                print('eps:{}, epoc:({}), iter:({}/{}), iters:({}/{})'.format(
                    et, epoch_, iters_, self.data_loader.train_iterations,
            ...
            if self.current_iteration % self.config.sample_step == 0:
                ...
                with torch.no_grad():
                    ...
                    for c_trg_sample in c_sample_list:
                        ...
                        x_fake_list.append(
                            self.G(x_sample, attr_diff.to(self.device)))
                    x_concat = torch.cat(x_fake_list, dim=3)
                    self.writer.add_image('sample', make_grid(self.denorm(
                        x_concat.data.cpu()), nrow=1), self.current_iteration)
                    save_image(self.denorm(x_concat.data.cpu()), os.path.join(
    ...
    def test(self):
        ...
        tqdm_loader = tqdm(self.data_loader.test_loader,
                           total=self.data_loader.test_iterations,
                           desc='Testing at checkpoint {}'.format(
        ...
        self.G.eval()
        with torch.no_grad():
            for i, (x_real, c_org, image_name) in enumerate(tqdm_loader):
                ...
                for img_tensor in x_fake_list:
                    result_path = os.path.join(self.config.result_dir,
                                               '{}.png'.format(image_name[0]))
                    save_image(self.denorm(img_tensor.data.cpu()),
